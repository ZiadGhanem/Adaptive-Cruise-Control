...
class laneTracker:
	def __init__(self, camera_info, topic_to, test_images_path):
		...
		dst = np.float32([
		    (self._camera_width - 175, 0),
		    (175, 0),
		    (175, self._camera_height),
		    (self._camera_width - 175, self._camera_height)
		])
		self._M = cv2.getPerspectiveTransform(src, dst)
		self._invM = cv2.getPerspectiveTransform(dst, src)

		# Create ROI mask
		vertices = np.array([
		    [150, self._camera_height],
		    [150, 200],
		    [500, 200],
		    [500, self._camera_height]
		])
		vertices = np.array(vertices, ndmin=3, dtype=np.int32)
		fill_color = (255,) * 3
		mask = np.zeros((self._camera_height, self._camera_width, 3)).astype(np.uint8)
		self._roi_mask = cv2.fillPoly(mask, vertices, fill_color)
		...
		preprocessed_image = self.get_image(test_images_path['right'])
		y_mppx1, x_mppx1 = self.compute_mppx(preprocessed_image, dashed_line_loc='right')

		preprocessed_image = self.get_image(test_images_path['left'])
	...
	def preprocess_image(self, img):
		...
		# 2. Prespective transformation
		warped = cv2.warpPerspective(img, self._M, (self._camera_width, self._camera_height), flags=cv2.INTER_LINEAR)	

		# 3. ROI crop
		roi = cv2.bitwise_and(warped, self._roi_mask)
	...
	def draw(self, img, warped, poly_param, curve_rad, offset):
		'''
		...
		unwarped = cv2.warpPerspective(color_warp, self._invM, (img.shape[1], img.shape[0]), flags=cv2.INTER_LINEAR)
	...
	def pipeline(self, data):
		try:
			...
			print(e)
		result = np.copy(img)
		warped = self.preprocess_image(img)
		...
		result = self.draw(img, warped, poly_param, (left_curverad + right_curverad) / 2, offset)

		try:
			self._lanetracker_publisher.publish(self._bridge.cv2_to_imgmsg(result, encoding="rgb8"))
		except CvBridgeError as e:
			print(e)
