...
class PID(QtWidgets.QWidget):
	...
	# PyQT Signals
	_throttle_scrollbar_signal = QtCore.pyqtSignal(int)
	_steering_scrollbar_signal = QtCore.pyqtSignal(int)
	_velocity_lcdnumber_signal = QtCore.pyqtSignal(int)
	_cruise_lcdnumber_signal = QtCore.pyqtSignal(int)
	_cruise_radiobutton_signal = QtCore.pyqtSignal(bool)
	_gears_lineedit_signal = QtCore.pyqtSignal(str)
	...
	def __init__(self,topics_from, topic_to):
		...
		self._throttle_scrollbar_signal.connect(self.throttle_scrollbar_callback)
		self._steering_scrollbar_signal.connect(self.steering_scrollbar_callback)
		self._velocity_lcdnumber_signal.connect(self.velocity_lcdnumber_callback)
		self._cruise_lcdnumber_signal.connect(self.cruise_lcdnumber_callback)
		self._cruise_radiobutton_signal.connect(self.cruise_radiobutton_callback)
		self._gears_lineedit_signal.connect(self.gears_lineedit_callback)
	...
	def throttle_scrollbar_callback(self, value):
		self.throttle_scrollbar.setValue(value)
	def steering_scrollbar_callback(self, value):
		self.steering_scrollbar.setValue(value)
	
	def velocity_lcdnumber_callback(self, value):
		self.velocity_lcdnumber.display(value)

	def cruise_lcdnumber_callback(self, value):
		self.cruise_lcdnumber.display(value)
	
	def cruise_radiobutton_callback(self, value):
		self.cruise_radiobutton.setChecked(value)

	def gears_lineedit_callback(self, value):
		self.gears_lineedit.setText(value)
	...
	def odometry_callback(self, odometry):
		...
		self._velocity_lcdnumber_signal.emit(int(self._current_velocity))

	def closest_distance_callback(self, closest_distance):
		self._closest_distance = max(0, closest_distance.data - 4)
	...
	def joystick_callback(self, message):
		...
		self._steering_scrollbar_signal.emit(int(message.axes[self._STEERING_AXIS]* -100))
		...
		if self._cruise_control_state:
			...
			if longitudinal_output >= 0:
				...
				command.brake = 0.0
				self._throttle_scrollbar_signal.emit(int(command.throttle * -100))
			else:
			...
			self._throttle_scrollbar_signal.emit(int(message.axes[self._THROTTLE_AXIS] * -100))
